...
function Analytics({stableId}){
  const [data,setData]=useState({finance:[],work:[],races:[],horses:[],invoices:[]});
  useEffect(()=>{if(stableId)load()},[stableId]);

  async function load(){
    const [finance,work,races,horses,invoices]=await Promise.all([
      ...
      supabase.from("invoices").select("*").eq("stable_id",stableId)
    ]);
    setData({
  ...
  function resultText(r){ return String(r.result||"").trim().toLowerCase(); }
  function isWin(r){
    const x=resultText(r);
    return x==="1" || x==="1st" || x.includes("win") || x.includes("won");
  }
  function isPlacing(r){
    const x=resultText(r);
    return ["1","1st","2","2nd","3","3rd"].includes(x) || x.includes("win") || x.includes("won");
  }

  const completed=data.races.filter(r=>r.status==="Completed");
  const totalPrizemoney=completed.reduce((s,r)=>s+Number(r.prizemoney||0),0);
  const financeIncome=data.finance.filter(f=>f.entry_type==="Income").reduce((s,f)=>s+Number(f.amount||0),0);
  const raceIncome=totalPrizemoney;
  const income=financeIncome+raceIncome;
  const expenses=data.finance.filter(f=>f.entry_type==="Expense").reduce((s,f)=>s+Number(f.amount||0),0);
  const net=income-expenses;
  const wins=completed.filter(isWin).length;
  const placings=completed.filter(isPlacing).length;
  const starts=completed.length;
  const winRate=starts?((wins/starts)*100).toFixed(1):"0.0";
  const placingRate=starts?((placings/starts)*100).toFixed(1):"0.0";

  return <main className="page">
    <section className="module-header">
      <div className="module-icon"><BarChart3 size={24}/></div>
      <div><h2>Analytics</h2><p>Stable income, expenses, net, work volume, wins, placings and prizemoney.</p></div>
    </section>
    ...
    <section className="stats analytics-stats">
      ...
      <Stat icon={BarChart3} label="Net" value={`$${net.toLocaleString()}`}/>
      <Stat icon={ClipboardList} label="Work Entries" value={data.work.length}/>
      <Stat icon={Trophy} label="Race Starts" value={starts}/>
      <Stat icon={Trophy} label="Wins" value={wins}/>
      <Stat icon={Trophy} label="Placings" value={placings}/>
      <Stat icon={DollarSign} label="Prizemoney" value={`$${totalPrizemoney.toLocaleString()}`}/>
    </section>
    ...
    <section className="analytics-panel">
      <div>
        ...
        <div className="analytics-bars">
          <div><span>Win Rate</span><strong>{winRate}%</strong><em style={{width:`${Math.min(100,Number(winRate))}%`}} /></div>
          <div><span>Placing Rate</span><strong>{placingRate}%</strong><em style={{width:`${Math.min(100,Number(placingRate))}%`}} /></div>
        </div>
      </div>
      <div>
        <h3>Income Breakdown</h3>
        <p>Finance income: <strong>${financeIncome.toLocaleString()}</strong></p>
        <p>Race prizemoney: <strong>${raceIncome.toLocaleString()}</strong></p>
      </div>
    </section>

    <section className="grid">
      {data.horses.map(h=>{
        const works=data.work.filter(w=>w.horse_name===h.name);
        const races=data.races.filter(r=>r.horse_name===h.name);
        const completedHorse=races.filter(r=>r.status==="Completed");
        const pm=completedHorse.reduce((s,r)=>s+Number(r.prizemoney||0),0);
        const hwins=completedHorse.filter(isWin).length;
        const hplacings=completedHorse.filter(isPlacing).length;
        const inv=data.invoices.filter(i=>i.horse_name===h.name).reduce((s,i)=>s+Number(i.total||0),0);
        return <article className="record" key={h.id}>
          <h3>{h.name}</h3>
          <div className="details">
            <div><span>Work Entries</span><strong>{works.length}</strong></div>
            <div><span>Starts</span><strong>{completedHorse.length}</strong></div>
            <div><span>Wins</span><strong>{hwins}</strong></div>
            <div><span>Placings</span><strong>{hplacings}</strong></div>
            <div><span>Prizemoney</span><strong>${pm.toLocaleString()}</strong></div>
            <div><span>Invoiced</span><strong>${inv.toLocaleString()}</strong></div>
          </div>
        </article>
      })}
    </section>
